...
The strategies part define the procedure of selecting the modules (classes), the routines (methods) and generation of values for instances involved in the routines. It contains all the test strategies for generation of test data. By default, YETI uses R strategy if no particular strategy is defined during test initialization. It allows the user to control the probability of using null values and the percentage of newly created objects for each test session. On top of the hierarchy in strategy part is an abstract class $YetiStrategy$, which is extended to form R strategy. The R strategy is extended to form R+ strategy. Finally, the DSSR strategy is an extension of R+ strategy. The class hierarchy is shown in Figure \ref{fig:hierarchyofDSSR}.
...
The improvement of DSSR over R and R$^+$ strategy is calculated by applying the formula (1) and (2) respectively. This is in accordance with the previous work of Chan et al.~\cite{chan2004good}.
