...
Jcrasher is first of the three automatic testing tools developed by Csallner C. and Smaragadakis Y. \cite{Pacheco2007b}. As the name suggests JCrasher tries to crash the Java program with random input and any exceptions thrown during the process are recorded. All obtained exceptions are then compared to the list of acceptable exception, which are defined in advance as heuristics, and any undefined/undeclared runtime exceptions are considered bugs. Since programs interact with the world through its public methods therefore JCrasher tests only these methods. 

\begin{figure}[h]
	\centering
	\includegraphics[width=6cm, height=4cm]{Literature/jcrasher.png}
	\caption{Process of testing a java program with JCrasher \cite{Pacheco2007b}}
\end{figure}
