...
As evident from the program code that an $ArithmeticException$ failure (divison by zero) is generated when the value of variable $x~is~one~of~\{4, 5, 6, 7, 8\}$ and the corresponding value of variable $y~is~one~of~\{2, 3, 4\}$. The values form a block failure-domain in the input domain. 
