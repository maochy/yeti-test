...
\title{Automated Discover of Failure Domain+}
...
\begin{enumerate}
\item Is ADFD+ and Randoop capable of identifying the failure?
\item Is ADFD+ and Randoop capable of identifying the failure domain?
\item Is ADFD+ and Randoop capable of identifying the boundaries of the failure domain?
% you will also prove it through figure but you can also say ADFD+ clarify it further by showing the boundaries in graphical form.
\end{enumerate}

\subsection{Experimental setup}
To evaluate the performance of ADFD+ and Randoop, we carried out testing of several error-seeded one and two-dimensional numerical programs written in Java. The programs were divided in to two sets. Set A and B contains one and two-dimensional programs respectively. Each program was injected with at least one failure domain of point, block or strip nature. Every program was tested independently for 30 times by both ADFD+ and Daikon on the basis of time and number of tests required to find all the failures. The external parameters were kept constant in each test. The detail of failure values and domain are given in Table~\ref{table:}. Due to the absence of contracts and assertions in the code under test, undeclared exceptions were taken as failures in accordance with the previous studies~\cite{ahmad2013adfd, Oriol2012}.
...
All tests were performed with a 64-bit Mac OS X Mountain lion version 10.8.5 running on 2.7 GHz Intel Core i7 with 16 GB (1600 MHz DDR3) of RAM. YETI runs on top of the Java\texttrademark  SE Runtime Environment [version 1.6.0\_35]. %The machine took approximately 100 hours to process the experiments.
...
%The results are split in to two sections for convenience. The first section ? contains results of the experiments performed by ADFD+ strategy and the second section ? contains results of Daikon on one and two dimensional numerical programs.  

%\subsection{ADFD+ results} \label{adfd+_results}
...
Write evaluation if any because you mentioned it in the structure of the paper.


\begin{figure}[ht]
\centering
\includegraphics[width=8.5cm,height=5.5cm]{timeADFD+vsRandoop.png}
\caption{Number of failures time}
\label{fig:timelimit}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=8.5cm,height=5.5cm]{NumberTestCasesADFD+vsRandoop.png}
\caption{Number of failures number of tests}
