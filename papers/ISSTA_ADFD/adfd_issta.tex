...
\indent The values from the pass and fail files are plotted on (x, y) chart using JFreeChart. JFreeChart is a free open-source java library that helps developers to display complex charts and graphs in their applications \cite{Gilbert2008}. Green colour represent pass values while red colour represent the faulty values. Resultant graph clearly depicts both the pass and fail domain. The graph shows red points in case the program fails for only one value, blocks for failing multiple values and strip for failing long range of values.\\
