...
\begin{document}
\title{ADFD+: An Automatic Testing Technique for Finding and Presenting Failure domains}
...
The Automated Discovery of Failure Domain (ADFD) and its successor Automated Discovery of Failure Domain+ (ADFD+) strategy, finds failures and their domains in a specified range and provides visualisation of the pass and fail domain. They can precisely detect the failure-domain of the identified failure in an effective way. Performing exhaustive testing in a limited region around the failure is the key to the success of ADFD and ADFD+.

Our studies include whether automated testing (ADFD and ADFD+) can replace Manual testing because they exhaustively test the SUT in specific range, whether the nature, location and size of failure-domain has an impact on the effectiveness of automated and manual testing, whether the number of failures and the underlying failure-domain types (point, block and strip) has an impact on the automated and manual testing. 
